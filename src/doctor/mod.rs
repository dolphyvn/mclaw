...
pub fn run(config: &Config) -> Result<()> {
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `mclaw doctor` again.");
    }
...
pub async fn run_models(
    ...
    println!("🩺 MClaw Doctor — Model Catalog Probe");
...
pub fn run_traces(
    ...
    println!();
    println!("Use `mclaw doctor traces --id <trace-id>` to inspect a full event payload.");
    Ok(())
}
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `mclaw onboard` to set one up",
        ));
    }
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".mclaw_doctor_probe_")));
    }
