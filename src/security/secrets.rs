...
// in `~/.mclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.mclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(mclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: mclaw_dir.join(".secret_key"),
            enabled,
        }
    }
