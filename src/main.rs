...
#[derive(Parser, Debug)]
#[command(name = "mclaw")]
#[command(author = "mclaw contributors")]
#[command(version)]
#[command(about = "Multi-tenant AI agent runtime with centralized LLM gateway.", long_about = None)]
struct Cli {
...
Examples:
  mclaw agent                              # interactive session
  mclaw agent -m \"Summarize today's logs\"  # single message
  mclaw agent -p anthropic --model claude-sonnet-4-20250514
  mclaw agent --peripheral nucleo-f401re:/dev/ttyACM0")]
    Agent {
...
Examples:
  mclaw gateway start              # start gateway
  mclaw gateway restart            # restart gateway
  mclaw gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
        gateway_command: Option<mclaw::GatewayCommands>,
    },
...
Launches the full MClaw runtime: gateway server, all configured \
channels (Telegram, Discord, Slack, etc.), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run MClaw in \
production or as an always-on assistant.

Use 'mclaw service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  mclaw daemon                   # use config defaults
  mclaw daemon -p 9090           # gateway on port 9090
  mclaw daemon --host 127.0.0.1  # localhost only")]
    Daemon {
    ...
    /// Examples:
    /// - `mclaw estop`
    /// - `mclaw estop --level network-kill`
    /// - `mclaw estop --level domain-block --domain "*.chase.com"`
    /// - `mclaw estop --level tool-freeze --tool shell --tool browser`
    /// - `mclaw estop status`
    /// - `mclaw estop resume --network`
    /// - `mclaw estop resume --domain "*.chase.com"`
    /// - `mclaw estop resume --tool shell`
    Estop {
        #[command(subcommand)]
        estop_command: Option<EstopSubcommands>,

        /// Level used when engaging estop from `mclaw estop`.
        #[arg(long, value_enum)]
...
Examples:
  mclaw cron list
  mclaw cron add '0 9 * * 1-5' 'Good morning' --tz America/New_York
  mclaw cron add '*/30 * * * *' 'Check system health'
  mclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  mclaw cron add-every 60000 'Ping heartbeat'
  mclaw cron once 30m 'Run backup in 30 minutes'
  mclaw cron pause <task-id>
  mclaw cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, and health-check channels that connect MClaw \
to messaging platforms. Supported channel types: telegram, discord, \
slack, whatsapp, matrix, imessage, email.

Examples:
  mclaw channel list
  mclaw channel doctor
  mclaw channel add telegram '{\"bot_token\":\"...\",\"name\":\"my-bot\"}'
  mclaw channel remove my-bot
  mclaw channel bind-telegram mclaw_user")]
    Channel {
        #[command(subcommand)]
...
Examples:
  mclaw hardware discover
  mclaw hardware introspect /dev/ttyACM0
  mclaw hardware info --chip STM32F401RETx")]
    Hardware {
        #[command(subcommand)]
        hardware_command: mclaw::HardwareCommands,
    },
...
Examples:
  mclaw peripheral list
  mclaw peripheral add nucleo-f401re /dev/ttyACM0
  mclaw peripheral add rpi-gpio native
  mclaw peripheral flash --port /dev/cu.usbmodem12345
  mclaw peripheral flash-nucleo")]
    Peripheral {
        #[command(subcommand)]
        peripheral_command: mclaw::PeripheralCommands,
    },
...
Examples:
  mclaw memory stats
  mclaw memory list
  mclaw memory list --category core --limit 10
  mclaw memory get <key>
  mclaw memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  mclaw config schema              # print JSON Schema to stdout
  mclaw config schema > schema.json")]
    Config {
        #[command(subcommand)]
        config_command: ConfigCommands,
    },

    /// Start the multi-tenant LLM gateway server
    #[command(long_about = "\
Start the multi-tenant LLM gateway server.

The gateway server centralizes LLM API keys and routes requests based on \
client identity. Different client groups can use different providers.

Examples:
  mclaw gateway-server                # use config defaults
  mclaw gateway-server --port 42618   # custom port
  mclaw gateway-server --host 0.0.0.0 # bind to all interfaces")]
    GatewayServer {
        /// Port to listen on (use 0 for random available port); defaults to multi_tenant.port or 42618
        #[arg(short, long)]
        port: Option<u16>,

        /// Host to bind to; defaults to multi_tenant.host or 0.0.0.0
        #[arg(long)]
        host: Option<String>,
    },

    /// Generate a new client secret for multi-tenant gateway authentication
    #[command(long_about = "\
Generate a new client secret for multi-tenant gateway authentication.

The generated secret can be used to authenticate a client group with the gateway.
Add the generated secret to your config under [multi_tenant.groups.<name>].

Examples:
  mclaw generate-secret group-a        # generate secret for group-a
  mclaw generate-secret production      # generate secret for production")]
    GenerateSecret {
        /// Client ID (group name)
        client_id: String,
    },

    /// List all configured multi-tenant gateway clients
    #[command(long_about = "\
List all configured multi-tenant gateway clients.

Shows all client groups configured in the multi-tenant gateway, including \
their assigned provider and model.
...
Examples:
  source <(mclaw completions bash)
  mclaw completions zsh > ~/.zfunc/_mclaw
  mclaw completions fish > ~/.config/fish/completions/mclaw.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
...
async fn main() -> Result<()> {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for MClaw.");
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(mclaw::GatewayCommands::Restart { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    let addr = format!("{host}:{port}");
                    info!("🔄 Restarting MClaw Gateway on {addr}");
                ...
                Some(mclaw::GatewayCommands::GetPaircode { new }) => {
                    ...
                    match fetch_paircode(host, port, new).await {
                        ...
                        Err(e) => {
                            ...
                            println!("     mclaw gateway start");
                        }
                    }
                    Ok(())
                }
                Some(mclaw::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    gateway::run_gateway(&host, port, config).await
                }
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting MClaw Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting MClaw Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 MClaw Status");
            println!();
        ...
        Commands::Config { config_command } => match config_command {
            ConfigCommands::Schema => {
                ...
                println!(
                    "{}",
                    serde_json::to_string_pretty(&schema).unwrap_or_else(|e| format!("{e}"))
                );
                Ok(())
            }
        },

        Commands::GatewayServer { port, host } => {
            let multi_tenant_config = config.multi_tenant.as_ref()
                .ok_or_else(|| anyhow::anyhow!("Multi-tenant gateway is not enabled. Add [multi_tenant] section to config.toml."))?;

            let port = port.unwrap_or(multi_tenant_config.port.unwrap_or(42618));
            let host = host.unwrap_or(multi_tenant_config.host.clone().unwrap_or_else(|| "0.0.0.0".to_string()));
            ...
            multi_tenant::server::run_gateway_server(config, host, port).await
        },

        Commands::GenerateSecret { client_id } => {
            let secret = multi_tenant::auth::generate_client_secret(&client_id);
            println!("Client ID: {}", client_id);
            println!("Client Secret: {}", secret);
            println!();
            println!("Add this to your config.toml under [multi_tenant.groups.{}]:", client_id);
            println!("  client_id = \"{}\"", client_id);
            println!("  client_secret = \"{}\"", secret);
            println!("  provider = \"openrouter\"");
            println!("  model = \"anthropic/claude-sonnet-4-20250514\"");
            Ok(())
        },

        Commands::ListClients => {
            let multi_tenant_config = config.multi_tenant.as_ref()
                .ok_or_else(|| anyhow::anyhow!("Multi-tenant gateway is not enabled."))?;

            println!("Configured Clients:");
            println!();

            if multi_tenant_config.groups.is_empty() {
                println!("  No clients configured.");
                println!("  Add clients under [multi_tenant.groups.<name>] in config.toml");
            } else {
                for (name, group) in &multi_tenant_config.groups {
                    println!("  [{}]", name);
                    println!("    Provider: {}", group.provider);
                    println!("    Model: {}", group.model);
                    println!("    Secret: {}***", &group.client_secret[..group.client_secret.len().min(20)]);
                    println!();
                }
            }
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting MClaw Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting MClaw Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `mclaw auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `mclaw auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `mclaw auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `mclaw auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `mclaw auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `mclaw auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["mclaw", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["mclaw", "onboard", "--force"])
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["mclaw", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["mclaw", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["mclaw", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["mclaw", "agent", "--message", "hello"])
