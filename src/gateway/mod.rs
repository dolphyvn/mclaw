...
pub async fn run_gateway(host: &str, port: u16, config: Config) -> Result<()> {
    // Check if gateway should run in dispatcher client mode
    if config.dispatcher.enabled && config.dispatcher.mode == "ws" {
        return self::dispatcher_mode::run_gateway_dispatcher_mode(config).await;
    }
    ...
    println!("🦀 MClaw Gateway listening on http://{display_addr}");
...
async fn handle_admin_shutdown(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    tokio::spawn(async move {
        ...
        tracing::info!("🦀 MClaw Gateway shutting down...");
        std::process::exit(0);
    });
...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_output() {
        ...
        let body = response.into_body().collect().await.unwrap().to_bytes();
        let text = String::from_utf8(body.to_vec()).unwrap();
        assert!(text.contains("mclaw_heartbeat_ticks_total 1"));
    }
