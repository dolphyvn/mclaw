...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/mclaw-pacman.XXXXXX.conf)"
...
Modes:
  Default mode installs/builds MClaw only (requires existing Rust toolchain).
...
Examples:
  ...
  # Remote one-liner
  curl -fsSL https://raw.githubusercontent.com/mclaw-labs/mclaw/main/install.sh | bash

Environment:
  MCLAW_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  MCLAW_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  MCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: mclaw-bootstrap:local)
  MCLAW_API_KEY           Used when --api-key is not provided
  MCLAW_PROVIDER          Used when --provider is not provided (default: openrouter)
  MCLAW_MODEL             Used when --model is not provided
  MCLAW_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  MCLAW_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  MCLAW_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${MCLAW_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${MCLAW_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
install_prebuilt_binary() {
  ...
  archive_url="https://github.com/mclaw-labs/mclaw/releases/latest/download/mclaw-${target}.tar.gz"
  temp_dir="$(mktemp -d -t mclaw-prebuilt-XXXXXX)"
  archive_path="$temp_dir/mclaw-${target}.tar.gz"
  ...
  extracted_bin="$temp_dir/mclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name mclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable mclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/mclaw"
  rm -rf "$temp_dir"

  info "Installed pre-built binary to $install_dir/mclaw"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/mclaw-pacman.XXXXXX.conf)"
...
run_guided_installer() {
  ...
  if prompt_yes_no "Install mclaw into cargo bin now?" "yes"; then
    SKIP_INSTALL=false
  else
    SKIP_INSTALL=true
  fi
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${MCLAW_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set MCLAW_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set MCLAW_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${MCLAW_DOCKER_IMAGE:-mclaw-bootstrap:local}"
  fallback_image="ghcr.io/mclaw-labs/mclaw:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.mclaw-docker"
  else
    default_data_dir="$WORK_DIR/.mclaw-docker"
  fi
  docker_data_dir="${MCLAW_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.mclaw" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official MClaw image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.mclaw:/mclaw-data/.mclaw"
  workspace_mount="$docker_data_dir/workspace:/mclaw-data/workspace"
...
Use either:
  --api-key "sk-..."
or:
  MCLAW_API_KEY="sk-..." ./install.sh --docker
or run interactive:
...
MSG
  ...
  "$CONTAINER_CLI" run --rm -it \
    ...
    -e HOME=/mclaw-data \
    -e MCLAW_WORKSPACE=/mclaw-data/workspace \
    -v "$config_mount" \
    -v "$workspace_mount" \
    "$docker_image" \
    "${onboard_cmd[@]}"
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/mclaw-labs/mclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${MCLAW_CONTAINER_CLI:-docker}"
API_KEY="${MCLAW_API_KEY:-}"
PROVIDER="${MCLAW_PROVIDER:-openrouter}"
MODEL="${MCLAW_MODEL:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${MCLAW_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set MCLAW_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t mclaw-bootstrap-XXXXXX)"
...
Your containerized MClaw data is persisted under:
DONE
...
if [[ "$SKIP_INSTALL" == false ]]; then
  info "Installing mclaw to cargo bin"
...
MCLAW_BIN=""
if have_cmd mclaw; then
  MCLAW_BIN="mclaw"
elif [[ -x "$HOME/.cargo/bin/mclaw" ]]; then
  MCLAW_BIN="$HOME/.cargo/bin/mclaw"
elif [[ -x "$WORK_DIR/target/release/mclaw" ]]; then
  MCLAW_BIN="$WORK_DIR/target/release/mclaw"
fi

if [[ "$RUN_ONBOARD" == true ]]; then
  if [[ -z "$MCLAW_BIN" ]]; then
    error "onboarding requested but mclaw binary is not available."
    error "Run without --skip-install, or ensure mclaw is in PATH."
    exit 1
  fi

  if [[ "$INTERACTIVE_ONBOARD" == true ]]; then
    info "Running interactive onboarding"
    "$MCLAW_BIN" onboard --interactive
  else
    if [[ -z "$API_KEY" ]]; then
      cat <<'MSG'
==> Onboarding requested, but API key not provided.
Use either:
  --api-key "sk-..."
or:
  MCLAW_API_KEY="sk-..." ./install.sh --onboard
or run interactive:
  ./install.sh --interactive-onboard
MSG
      exit 1
    ...
    ONBOARD_CMD=("$MCLAW_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    "${ONBOARD_CMD[@]}"
